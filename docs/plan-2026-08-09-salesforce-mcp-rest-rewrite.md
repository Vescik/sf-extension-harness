...
| F-4 | Usunięcie `.mjs` + pinu `@salesforce/mcp` + cache paragonów; pełny gate (`validate_harness.py`, unittest, `run_evals.py`); **weryfikacja żywa** na wolnym orgu: sekwencja wielu calli, która dziś reprodukuje timeout; wpisy do `.ai/memory/decisions-log.md` (D-1…D-6) i RESULTS-LOG w warsztacie | ~0,5–1 dzień |
