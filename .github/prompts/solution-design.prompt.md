...
`work-item/<id>-<slug>` agreeing with the persisted folder, or the combined Feature branch
`feature/<feature-id>-<slug>` whose single prepared delivery map lists the item as included. If
design is invoked from `main` or an unrelated branch, write nothing and return
