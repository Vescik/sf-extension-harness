...
tools: ['read', 'search', 'edit/editFiles', 'execute/runInTerminal', 'vscode/askQuestions', 'salesforce/review_org_identity', 'salesforce/review_installed_packages', 'salesforce/review_object_contract', 'salesforce/review_configured_orgs', 'salesforce/review_soql_query', 'knowledge/*']
---
...
grounded through the guarded review tools (record reads via `review_soql_query`); the facade proves the non-production org identity before tool discovery, and object facts reconcile the describe and Tooling endpoints with contested traits nulled and listed in `contestedProperties`.
